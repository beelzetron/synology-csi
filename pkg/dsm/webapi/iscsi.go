...
// LunFindByName returns the LUN whose Name matches, or NoSuchLunError if none.
// Use this when the identifier is a LUN name; LunGet sends "uuid" and expects a LUN UUID.
func (dsm *DSM) LunFindByName(name string) (LunInfo, error) {
	luns, err := dsm.LunList()
	if err != nil {
		return LunInfo{}, err
	}
	for _, l := range luns {
	...
	return LunInfo{}, utils.NoSuchLunError("")
}
